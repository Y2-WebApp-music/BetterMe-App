...
const Inprogress = () => {
  ...
  const getGoal = async () => {
    try {
      const response = await axios.get(`${SERVER_URL}/goal/in-progress/${user?._id}`);
      const data = response.data // homeGoalCardProp[]

      console.log('getGoal response \n',response.data);

      if ((data.message === "Goal not found") || (data.message === "No in progress goals")) {
        setIsEmpty(true)
      } else {
        setGoal([
          ...data.map((goal: any) => ({
    ...
    <SafeAreaView style={{backgroundColor:colors.background}} className="w-full h-full justify-start items-center font-noto" >
      <View className='w-[92%] mt-4'>
        ...
        <View className='flex flex-row gap-2 items-center mt-2'>
          <View className='grow'>
            <Text className='text-subTitle text-yellow font-notoMedium'>Inprogress goal</Text>
          </View>
        </View>
      </View>
      <ScrollView
      ...
      >
        <View className='w-full'>
          {isEmpty? (
            ...
            <View className='mt-2 flex-col gap-2 w-full'>
              <FlashList
                data={goal}
                renderItem={({ item }) =>
