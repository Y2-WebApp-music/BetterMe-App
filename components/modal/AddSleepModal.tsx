...
const AddSleepModal = ({date, startTime, setStartTime, endTime, setEndTime, isOpen, setIsOpen, createTime}:EditSleepModalProp) => {
  ...
  const durationHour = useDerivedValue(() => {
    ...
    return formatDurationHour(radToMinutes(d)+ 390);
  });
  const durationMinute = useDerivedValue(() => {
    const d = absoluteDuration(start.value, end.value);
    return formatDurationMinute(radToMinutes(d)+ 390 );
  });

  useFocusEffect(
    useCallback(() => {
      if (isOpen) {
        start.value = timeToAngle(startTime);
        end.value = timeToAngle(endTime);
      }
